...
def test_db_connection(req: DataSourceBase):
    try:
        ...
        if req.type.lower() in ("snmp", "socket"):
            return BaseResponse(code=0, msg="SNMP/Socket 采集不需要测试连接，请在任务中配置相关参数后直接执行")
...
def get_datasource_tables(req: DataSourceIdReq, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if source.type.lower() in ("snmp", "socket"):
            return BaseResponse(code=0, msg="SNMP/Socket 采集不支持表查询")
...
def get_datasource_tables_detail(req: DataSourceIdReq, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if source.type.lower() in ("snmp", "socket"):
            return BaseResponse(code=0, msg="SNMP/Socket 采集不支持表结构查询")
